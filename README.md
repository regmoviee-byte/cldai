...
### Windows: один файл aihub.exe

Скачай `aihub.exe` (последняя сборка лежит в разделе **Actions → Windows exe → Artifacts**)
и запусти двойным щелчком. Откроется обычное окно приложения со своей иконкой на панели задач,
без браузера и без чёрных консольных окон. Python и всё остальное уже внутри.

- При первом запуске в разделе «Агенты» нажми **Установить** у Claude Code и Codex CLI (это
  консольные версии Claude и ChatGPT, они работают по тем же подпискам), потом **Войти**:
  откроется браузер со входом в твой аккаунт. Всё идёт в фоне, прогресс виден в приложении.
- Файл без цифровой подписи, поэтому Windows может показать «Система Windows защитила ваш компьютер».
  Жми «Подробнее» → «Выполнить в любом случае».
- Закрыл окно — aihub остановился, вместе с запущенными задачами. Второй запуск просто выводит уже
  открытое окно на передний план.
- Окно работает на WebView2, встроенном в Windows 10/11. Если его нет, aihub откроется в окне
  Microsoft Edge без вкладок и адресной строки.
- Данные и логи лежат в `%LOCALAPPDATA%\aihub` (лог сервера: `aihub.log`).

Собрать exe самому (на Linux, macOS или Windows с Go): `./launcher/build.sh` → `dist/aihub.exe`.
