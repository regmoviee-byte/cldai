...
и запусти двойным щелчком. Python внутри, ставить его не нужно. Нужны только сами агенты:
[Claude Code](https://claude.com/claude-code) и/или [Codex CLI](https://github.com/openai/codex),
установленные и с выполненным входом.

- Первый запуск распаковывает программу в `%LOCALAPPDATA%\aihub` (несколько секунд), потом открывается браузер.
...
- Чёрное окно — это сам aihub: пока пользуешься, не закрывай его. Закрыл — программа остановилась.
- Если запустить второй раз, просто откроется уже работающее окно в браузере.
